...
class Bsu extends MY_Controller {
    
    public function __construct(){
        parent::__construct();
        date_default_timezone_set('UTC'); 
        $this->load->helper('url');
        $this->load->model(array('bsu_model'));
    }

    function index(){
            
        $this->data['title'] = 'Master Data - Entitas Bisnis';
        $this->data['content'] = 'bsu/index';
//        $this->data['dataList'] = $this->bsu_model->getByCategory(array('bse_ctgr'=>'left'));
//        if ($this->session->userdata('user_group') == '00') {
//            $this->data['dataList'] = $this->bsu_model->getByCategory();
//        } else {
//            $this->data['dataList'] = $this->bsu_model->getData($this->_limit,'',array('bse_type'=>'frontend'),'where');
//        }
        
        $this->data['dataList'] = $this->bsu_model->getData($this->_limit,'',array(),'where');
        $this->data['totaldata'] = sizeof($this->bsu_model->getAll());
        $this->data['pnumber'] = 1;
    ...
    function paging() {
        ...
        $totalData = $this->bsu_model->getData(0,$limit,$whereSearch,'or_like');
        $result = $this->bsu_model->getData($this->_limit,$limit,$whereSearch,'or_like');
    ...
    function add() {
        ...
        if ($this->uri->segment(3)) { //edited
            ...
            $dataTables = $this->bsu_model->getByCategory($where);
...
//            $this->data['bseType'] = $this->bseType;
    ...
    function save() {
        ...
        if ($new) { // add
            $id = $this->bsu_model->add($postData);
            if (!$id) {
                $success = false;
            ...
            $this->bsu_model->update($postData);
        }
    ...
    function delete() {
        ...
        if (sizeof($data) > 0) {
            foreach($data as $value) {
                $this->bsu_model->delete($this->bsu_model->_table,array('id'=>$value['id']));
