...
class Bse extends MY_Controller {
    ...
    function index(){
        ...
        $this->data['title'] = 'Master Data - Business Entity';
        $this->data['content'] = 'bse/index';
//        $this->data['dataList'] = $this->bse_model->getByCategory(array('bse_ctgr'=>'left'));
//        if ($this->session->userdata('user_group') == '00') {
//            $this->data['dataList'] = $this->bse_model->getByCategory();
//        } else {
//            $this->data['dataList'] = $this->bse_model->getData($this->_limit,'',array('bse_type'=>'frontend'),'where');
//        }
        
        $this->data['dataList'] = $this->bse_model->getData($this->_limit,'',array(),'where');
        $this->data['totaldata'] = sizeof($this->bse_model->getAll());
        $this->data['pnumber'] = 1;
    ...
    function paging() {
        ...
        $totalData = $this->bse_model->getData(0,$limit,$whereSearch,'or_like');
        $result = $this->bse_model->getData($this->_limit,$limit,$whereSearch,'or_like');
    ...
    function add() {
        ...
        if ($this->uri->segment(3)) { //edited
            ...
            $dataTables = $this->bse_model->getByCategory($where);
...
//            $this->data['bseType'] = $this->bseType;
    ...
    function save() {
        ...
        if ($new) { // add
            $id = $this->bse_model->add($postData);
            if (!$id) {
                $success = false;
            ...
            $this->bse_model->update($postData);
        }
    ...
    function delete() {
        ...
        if (sizeof($data) > 0) {
            foreach($data as $value) {
                $this->bse_model->delete($this->bse_model->_table,array('id'=>$value['id']));
